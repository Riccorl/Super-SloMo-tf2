...
class SloMoNet(tf.keras.Model):
    def __init__(self, batch_size, name="SloMoNet", **kwargs):
        super(SloMoNet, self).__init__(name=name, **kwargs)
        # t_slices = tf.constant(np.array([np.linspace(0, 1, 12)] * batch_size))
        t_slices = tf.constant(np.linspace(0, 1, 12))
    ...
    def call(self, inputs, training=False, **kwargs):
        frames_0, frames_t, frames_1, frames_i = inputs
